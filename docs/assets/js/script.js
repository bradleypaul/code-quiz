...
function generateAnswerElements(answers, correctAnswer) {
    const answersElement = document.querySelector("#answers");
    answersElement.innerText = "";
    answers.forEach((ans, index) => {
        const answerElement = generateAnswerElement(ans, index, correctAnswer);
        answersElement.append(answerElement);
    });
    answersElement.addEventListener("click", validateAnswer);
}

function validateAnswer(event) {
    const attribute = event.target.getAttribute("data-correct");
    if (attribute === "false") {
        timer -= 10;
        timeElement.innerText = timer;
        updateResult(false);
...
function saveScore(timer) {
    document.querySelector('#final-score').innerText = timer;
...
function createScoreElement(initials, score) {
    let scoreElement = document.createElement('li');
    scoreElement.innerText = `${initials} ${score}`;
    return scoreElement;
}
